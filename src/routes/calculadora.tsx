...
function calculateMacros(
  ...
  const objectiveAdjustments: Record<Objective, number> = {
    lose: -500,
    maintain: 0,
    gain: 300,
  };
  ...
  // Protein: 2g per kg (higher for active/gain, 1.8g for lose)
...
export default function Calculadora() {
  ...
  function handleCalculate(e: React.FormEvent) {
    e.preventDefault();
    const r = calculateMacros(sex, weight, height, age, activity, objective);
    setResult(r);
    setApplied(false);
  }
    ...
    <AppShell>
      ...
      <section className="mt-5 grid gap-4 md:grid-cols-2">
        <form onSubmit={handleCalculate} className="rounded-2xl border border-border/60 bg-card p-5 shadow-card">
          <h2 className="font-display text-lg font-semibold">Tu perfil</h2>
          ...
          <div className="mt-3 space-y-3">
            <div className="flex gap-2">
              ...
              <button
                ...
                className={btnToggle(sex === "female")}
              >
                Mujer
              </button>
            </div>
          ...
          <button
            type="submit"
            className="mt-4 w-full rounded-xl bg-gradient-primary py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition hover:opacity-90"
          >
            Calcular macros
          </button>
        </form>
            ...
            <div className="mt-3 space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={applied}
                  className="flex-1 rounded-xl bg-gradient-primary py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition hover:opacity-90 disabled:opacity-50"
                >
                  {applied ? (
                ...
                <Link
                  to="/macros"
                  className="inline-flex items-center gap-1 rounded-xl border border-border bg-background/60 px-4 py-2.5 text-sm font-medium text-foreground hover:bg-muted"
                >
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <input
        ...
        className="w-full rounded-lg border border-border bg-background/60 px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary"
      />
    </label>
  );
}
